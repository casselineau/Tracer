...
def rect_one_sided_mirror(width, height, absorptivity=0, sigma=0., bi_var=True, option=None, location=None, rotation=None, bounds=None):
	"""
	...
	"""
	if option == 'fast':
		surf = Surface(RectPlateGM(width, height), opt.OneSidedRealReflective(absorptivity, sigma, bi_var))
	else:
		surf = Surface(RectPlateGM(width, height), opt.OneSidedRealReflectiveDetector(absorptivity, sigma, bi_var))
	obj = AssembledObject(surfs=[surf], location=location, rotation=rotation, bounds=bounds)

	return obj


def rect_para_one_sided_mirror(width, height, focal_length, absorptivity=0., sigma=0., bi_var=True, option=None, location=None, rotation=None, bounds=None):

	if option == 'fast':
		surf = Surface(RectangularParabolicDishGM(width, height, focal_length),
				   opt.OneSidedRealReflective(absorptivity, sigma, bi_var))
	else:
		surf = Surface(RectangularParabolicDishGM(width, height, focal_length),
			opt.OneSidedRealReflectiveDetector(absorptivity, sigma, bi_var))
...
def flat_quad_one_sided_mirror(width, height, quad_params, absorptivity=0., sigma=0., bi_var=True, option=None, location=None, rotation=None, bounds=None):
	...
	if option == 'fast':
		...
		surf = Surface(RectFlatQuadricSurfaceGM(width, height, a, b, c, d, e, f),
			opt.OneSidedRealReflectiveReceiver(absorptivity, sigma, bi_var))
	else:
		surf = Surface(RectFlatQuadricSurfaceGM(width, height, a, b, c, d, e, f),
			opt.OneSidedRealReflectiveDetector(absorptivity, sigma, bi_var))
...
def one_sided_receiver(width, height, absorptivity=1, location=None, rotation=None):
	...
	obj = AssembledObject(surfs=[front], location=location, rotation=rotation)
